...
missing_classes = ['ItemRPGBow', 'ComponentRegistry', 'ComponentLawOfCycles', 'ComponentTrueLawOfCycles', 'EntityMadokaArrow', 'EntityMadokaRing', 'SetBonusManager', 'RenderMadokaRing', 'RenderMadokaArrow', 'RenderRPGBow', 'EntityVampire']
...
def extract_from_string(text):
    for mc in missing_classes:
        # Just find "public class <mc>" and try to grab everything around it
        idx = text.find('class ' + mc)
        if idx == -1: continue
        
        # look backwards for "package "
        pkg_idx = text.rfind('package com.greyhat.dark_grey', 0, idx)
        if pkg_idx == -1: continue
        ...
        if len(balanced_code) > len(file_map.get(mc, '')):
            file_map[mc] = balanced_code
