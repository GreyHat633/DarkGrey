...
output_dir = r'E:\Java\MinecraftMod\DarkGrey\temp_recover\com\greyhat\dark_grey'
...
def extract_from_string(text):
    matches = re.finditer(r'(package com\.greyhat\.dark_grey.*?;.*?public (?:final )?(?:class|interface) (\w+).*?{.*)', text, re.DOTALL)
    for m in matches:
        code = m.group(1)
        name = m.group(2)
...
def scan_file(path):
    print(f'Scanning jsonl in {path}')
    try:
...
print(f'Found {len(file_map)} classes across all brains.')
