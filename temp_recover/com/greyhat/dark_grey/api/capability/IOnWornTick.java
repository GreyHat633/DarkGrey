 ...
 *
 * <p>Triggered by {@code ItemArmor.onArmorTick()}, but <b>throttled by the container</b>
 * to fire only once per second (every 20 game ticks) to prevent tick storms.</p>
 *
 * <p>In Godot terms, this is equivalent to using a {@code Timer} node with a 1-second
 * interval attached to your armor node, rather than running logic every {@code _process()} frame.</p>
 *
 * <p>All calls happen server-side only ({@code !world.isRemote}).</p>
 */
