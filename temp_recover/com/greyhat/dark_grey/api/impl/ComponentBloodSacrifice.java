...
public class ComponentBloodSacrifice implements IRPGComponent, IOnHit, IHasTooltip {
    ...
    public void configure(JsonObject params) {
        if (params.has("min_crit_multiplier")) {
            minCritMultiplier = params.get("min_crit_multiplier").getAsFloat();
        }
        if (params.has("max_crit_multiplier")) {
            maxCritMultiplier = params.get("max_crit_multiplier").getAsFloat();
        }
        if (params.has("backlash_health_percentage")) {
            backlashHealthPercentage = params.get("backlash_health_percentage").getAsFloat();
        }
    }
    ...
    @Override
    public float onHit(ItemStack weaponStack, EntityLivingBase attacker, EntityLivingBase target, float baseDamage) {
        ...
        if (rand.nextFloat() < 0.15f) {
            ...
            if (attacker.getHealth() <= backlash) {
                ...
                attacker.attackEntityFrom(DamageSource.magic.setDamageBypassesArmor().setDamageIsAbsolute(), attacker.getMaxHealth() * 1000f);
            } else {
                // Normal damage
                attacker.attackEntityFrom(DamageSource.magic.setDamageBypassesArmor().setDamageIsAbsolute(), backlash);
            }
