...
public class LifestealComponent implements IRPGComponent, IOnHit, IHasTooltip {
    ...
    public void configure(JsonObject params) {
        if (params.has("percent")) {
            lifestealPercent = params.get("percent").getAsFloat();
        }
    }

    @Override
    public void onHit(ItemStack weaponStack, EntityLivingBase attacker,
    ...
    @Override
    public void addTooltipLines(ItemStack itemStack, EntityPlayer player,
                                List tooltipLines, boolean showAdvanced) {
        int displayPercent = Math.round(lifestealPercent * 100);
        // §4◆ 吸血 §7| §c+15% 攻击伤害转化为生命值
        tooltipLines.add("\u00A74\u25C6 \u5438\u8840 \u00A77| \u00A7c+" 
            + displayPercent + "% \u653B\u51FB\u4F24\u5BB3\u8F6C\u5316\u4E3A\u751F\u547D\u503C");
    }
}