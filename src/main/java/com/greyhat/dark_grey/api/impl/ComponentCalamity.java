...
public class ComponentCalamity implements IRPGComponent, IOnRightClick, IHasTooltip {
    ...
    public void addTooltipLines(ItemStack stack, EntityPlayer player, List<String> tooltip, boolean advanced) {
        // Output: §5✦ 劫难 §7| §d右键释放360度毁灭横扫 (冷却7秒)
        tooltip.add("\u00A75\u2726 \u52AB\u96BE \u00A77| \u00A7d\u53F3\u952E\u91CA\u653E360\u5EA6\u6BC1\u706D\u6A2A\u626B (\u51B7\u53747\u79D2)");
    }
    ...
    public ItemStack onRightClick(ItemStack weaponStack, World world, EntityPlayer player) {
        ...
        // Check if the item is already on cooldown (cooldown logic managed by vanilla cooldown or custom NBT, 
        // since 1.7.10 doesn't have native ItemCooldowns, we can use a custom tag or use an existing cooldown system.
        // Actually, let's use the player's NBT to store the last usage time for this skill, or rely on a standard implementation if one exists.
        // Wait, for 1.7.10, we can just use the NBT of the itemstack to track the cooldown.
        long current_time = world.getTotalWorldTime();
        long last_used_time = 0;
        
        if (weaponStack.hasTagCompound() && weaponStack.getTagCompound().hasKey("calamity_last_used")) {
            last_used_time = weaponStack.getTagCompound().getLong("calamity_last_used");
        }
        ...
        if (current_time - last_used_time < COOLDOWN_TICKS) {
            if (!world.isRemote) {
                ...
                player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "【劫难】技能冷却中，还需等待 " + String.format("%.1f", remaining_seconds) + " 秒。"));
            }
            return weaponStack;
        }
        ...
        weaponStack.getTagCompound().setLong("calamity_last_used", current_time);
