 ...
 *
 * <p>In Godot terms, think of this as the base {@code Resource} script that every
 * custom component must extend. Just as every Godot Resource has {@code _init()} and
 * serialization hooks, every RPG component has {@link #configure} and NBT methods.</p>
 *
 * <p>Concrete components implement this interface plus one or more capability sub-interfaces
 * (e.g. {@link com.greyhat.dark_grey.api.capability.IOnHit}) to declare what triggers they respond to.</p>
 */
public interface IRPGComponent {
     ...
     *
     * <p>Override this to read component-specific settings. The default implementation
     * does nothing, which is appropriate for parameterless components.</p>
     *
     ...
     *
     * <p>Stateless components (e.g. lifesteal) need not override this.
     * Stateful components (e.g. cooldown timers, stored XP) must override
     * both this and {@link #readFromNBT}.</p>
     *
     ...
     *
     * <p>This is analogous to Godot's {@code get_children().filter(func(node): node is SomeType)}
     * pattern — it partitions a flat component list by capability at construction time,
     * so runtime dispatch is a simple list iteration with zero reflection.</p>
     *
