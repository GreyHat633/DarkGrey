...
public class ComponentLawOfCycles implements IOnBowShoot, IHasTooltip, IOnBowUsingTick
{
    ...
    public void configure(final JsonObject params) {
        if (params != null) {
            this.damage1 = (params.has("damage1") ? params.get("damage1").getAsFloat() : 20.0f);
            this.damage2 = (params.has("damage2") ? params.get("damage2").getAsFloat() : 30.0f);
            this.damage3 = (params.has("damage3") ? params.get("damage3").getAsFloat() : 50.0f);
        }
    }
    ...
    @Override
    public boolean onBowShoot(final ItemStack bowStack, final World world, final EntityPlayer player, final int charge) {
        ...
        if (player.capabilities.isCreativeMode) {
            ...
            for (int i = 0; i < player.inventory.mainInventory.length; ++i) {
                if (player.inventory.mainInventory[i] != null && player.inventory.mainInventory[i].getItem() instanceof IRPGItemContainer) {
                    final String ammoId = ((IRPGItemContainer)player.inventory.mainInventory[i].getItem()).getRpgItemId();
                    final RPGItemDataManager.ItemConfig ammoConfig = RPGItemDataManager.getInstance().getConfig(ammoId);
        ...
        if (!world.isRemote) {
            ...
            world.playSoundAtEntity((Entity)player, "random.bow", 1.0f, 1.0f / (world.rand.nextFloat() * 0.4f + 1.2f) + velocity * 0.5f);
        }
        if (!player.capabilities.isCreativeMode && EnchantmentHelper.getEnchantmentLevel(Enchantment.infinity.effectId, bowStack) == 0) {
            for (int j = 0; j < player.inventory.mainInventory.length; ++j) {
                if (player.inventory.mainInventory[j] != null && player.inventory.mainInventory[j].getItem() instanceof IRPGItemContainer) {
                    final String ammoId2 = ((IRPGItemContainer)player.inventory.mainInventory[j].getItem()).getRpgItemId();
                    final RPGItemDataManager.ItemConfig ammoConfig2 = RPGItemDataManager.getInstance().getConfig(ammoId2);
                    if (ammoConfig2 != null && ("arrow".equals(ammoConfig2.type) || "\u7bad".equals(ammoConfig2.type))) {
    ...
    @Override
    public void onBowUsingTick(final ItemStack bowStack, final World world, final EntityPlayer player, final int count) {
    ...
    public void addTooltipLines(final ItemStack stack, final EntityPlayer player, final List<String> tooltip, final boolean advanced) {
        ...
        tooltip.add(String.format("  \u00A77\u25b6 \u00A7e1\u6bb5\u84c4\u529b \u00A78(1.0s)\u00A77: \u00A7c%.1f \u00A77\u70b9\u4f24\u5bb3", this.damage1));
        tooltip.add(String.format("  \u00A77\u25b6 \u00A762\u6bb5\u84c4\u529b \u00A78(2.0s)\u00A77: \u00A7c%.1f \u00A77\u70b9\u4f24\u5bb3", this.damage2));
        tooltip.add(String.format("  \u00A77\u25b6 \u00A7c\u00A7l3\u6bb5\u84c4\u529b \u00A78(3.0s)\u00A77: \u00A7c\u00A7l%.1f \u00A77\u70b9\u4f24\u5bb3", this.damage3));
    }
}
