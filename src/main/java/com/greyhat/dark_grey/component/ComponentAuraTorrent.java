...
public class ComponentAuraTorrent implements IRPGComponent, IOnRightClick, IOnHeldTick, IHasTooltip, IOnWeaponUsingTick, IOnPlayerStoppedUsing {
    ...
    public void configure(JsonObject params) {
        if (params.has("radius")) {
            radius = params.get("radius").getAsFloat();
        }
        if (params.has("dotDamage")) {
            dotDamage = params.get("dotDamage").getAsFloat();
        }
        if (params.has("cooldown")) {
            cooldownSeconds = params.get("cooldown").getAsInt();
        }
    }

    @Override
    public void addTooltipLines(ItemStack stack, EntityPlayer player, List<String> tooltip, boolean advanced) {
        tooltip.add("\u00A76\u2726 \u7075\u6C14\u6D2A\u6D41 \u00A77| \u00A7e\u53F3\u952E\u957F\u6309\u84C4\u529b\uFF0C\u677e\u5F00\u91CA\u653E");
        tooltip.add("\u00A77   \u00A78(\u84C4\u529b\u6700\u591A2\u79D2\u8FBE\u5230\u6700\u5927\u534A\u5F84 " + radius + "\uFF0C\u9635\u5185\u6BCF0.5\u79D2" + (int)dotDamage + "\u70B9\u4F24\u5BB3\uFF0C\u51B7\u5374" + cooldownSeconds + "\u79D2)");
    }
    ...
    public ItemStack onRightClick(ItemStack itemStack, World world, EntityPlayer player) {
        ...
        if (itemStack.hasTagCompound() && itemStack.getTagCompound().hasKey("LastAuraTorrentTime")) {
            lastTime = itemStack.getTagCompound().getLong("LastAuraTorrentTime");
        }
        ...
        if (currentTime - lastTime < cooldownTicks) {
            if (!world.isRemote) {
                ...
                player.addChatMessage(new net.minecraft.util.ChatComponentText(net.minecraft.util.EnumChatFormatting.RED + "【灵气洪流】技能冷却中，还需等待 " + String.format("%.1f", remaining_seconds) + " 秒。"));
            }
    ...
    public void onUsingTick(ItemStack weaponStack, EntityPlayer player, int count) {
        ...
        int charge = weaponStack.getItem().getMaxItemUseDuration(weaponStack) - count;
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World world, EntityPlayer player, int timeLeft) {
        int charge = stack.getItem().getMaxItemUseDuration(stack) - timeLeft;
        ...
        if (!world.isRemote) {
            ...
            EntityAuraTorrent aura = new EntityAuraTorrent(world, player, player.posX, spawnY, player.posZ, finalRadius, dotDamage);
            world.spawnEntityInWorld(aura);
            
            if (!stack.hasTagCompound()) {
                stack.setTagCompound(new NBTTagCompound());
            }
            stack.getTagCompound().setLong("LastAuraTorrentTime", world.getTotalWorldTime());
        }
    }
