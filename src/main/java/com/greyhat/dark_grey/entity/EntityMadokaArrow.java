...
public class EntityMadokaArrow extends EntityThrowable
{
    ...
    public EntityMadokaArrow(final World world, final EntityLivingBase shooter, final int chargeLevel, final float damage) {
        super(world, shooter);
        this.customDamage = 10.0f;
        this.hasSpawnedHexagrams = false;
        ...
        if (this.worldObj.isRemote) {
            ...
            if (level >= 3) {
                for (int i = 0; i < 30; ++i) {
                    ...
                    this.worldObj.spawnParticle("reddust", centerX + offsetX, centerY + offsetY, centerZ + offsetZ, 1.0, 0.9, 1.0);
                }
            }
        }
    ...
    private void spawnHexagramLayers() {
        ...
        Vec3 origin;
        if (shooter != null) {
            forward = shooter.getLook(1.0f).normalize();
            origin = Vec3.createVectorHelper(shooter.posX + forward.xCoord * 1.5, shooter.posY + shooter.getEyeHeight() + forward.yCoord * 1.5, shooter.posZ + forward.zCoord * 1.5);
        }
        else {
            forward = Vec3.createVectorHelper(this.motionX, this.motionY, this.motionZ).normalize();
        ...
        for (int layer = 0; layer < layers; ++layer) {
            ...
            final Vec3 center = Vec3.createVectorHelper(origin.xCoord + forward.xCoord * layerOffset, origin.yCoord + forward.yCoord * layerOffset, origin.zCoord + forward.zCoord * layerOffset);
    ...
    private void drawHexagram(final Vec3 center, final Vec3 right, final Vec3 up, final double radius) {
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            p[i] = Vec3.createVectorHelper(center.xCoord + right.xCoord * dx + up.xCoord * dy, center.yCoord + right.yCoord * dx + up.yCoord * dy, center.zCoord + right.zCoord * dx + up.zCoord * dy);
        }
    ...
    protected void onImpact(final MovingObjectPosition mop) {
        if (!this.worldObj.isRemote) {
            ...
            if (explosionSize > 0.0f) {
                ...
                if (level >= 2) {
                    this.worldObj.createExplosion(this.getThrower(), this.posX, this.posY - 1.5, this.posZ, explosionSize * 0.8f, true);
                }
            }
            this.setDead();
        }
